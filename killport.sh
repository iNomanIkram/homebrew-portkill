...
print_help() {
echo -e "${BLUE}portkill - advanced port process manager (v${VERSION})${NC}

Usage:
  portkill kill <port>           Kill a single port
  portkill killm <p1,p2,...>     Kill multiple ports
  portkill list                  List all listening ports
  portkill self                  List self-assigned ports (>1024)
  portkill version               Show version
  portkill help                  Show help
"
}

print_version() {
  echo "portkill version $VERSION"
}
